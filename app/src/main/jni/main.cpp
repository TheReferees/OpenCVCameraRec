...
#define APPNAME "CameraRecognition"
...
bool setYUVColors() {
    FILE *in;
    if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "LOADING....");
    ...
    while(fgets(buf, 256, in)) {
        ...
        if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "SCANNED COLORS: %i, %i, %i, %i, %i, %i", y1, y2, u1, u2, v1, v2);
...
void labelCells(cv::Mat& image, std::vector<unsigned int> labelBuffer, std::vector<int> * labelTable, std::vector<region> * regionsTable) {
    //if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "LABEL1");
    int width = image.cols;
    int height = image.rows;
    //if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "LABEL2");

    //Current label # to use
    int currLabel = 1;
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            //if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "LABEL3");

            //current pixel number in one dimensional array
            int i = y * width + x;
            //if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "LABEL4");
            ...
            //if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "LABEL5");

            //if the pixel matches a known color
            if (pixelColors[i] != 0) {
                //min: smallest label number of neighbors (A, B, C, D)
                int min = width * height + 1;
                //if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "LABEL6");
                ...
                //if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "LABEL7");
                ...
                if (min == width * height + 1) {
                    ...
                    //if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "LABEL8");

                    //Add the new label number to the label table
                    labelTable->push_back((int) labelTable->size());
                    region r;
                    //if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "LABEL9");
                    ...
                    //if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "LABEL10");
...
void findRegions(cv::Mat& image) {
    if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "REGIONS0");

    int width = image.cols;
    int height = image.rows;
    if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "REGIONS1");
    ...
    if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "REGIONS2: [%i, %i, %i]", width, height, width * height);
    ...
    if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "REGIONS3");
    labelCells(image, labelBuffer, &labelTable, &regionsTable);
    if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "REGIONS4");

    for (int i = 1; i < (int) regionsTable.size() - 1; i++) {
        if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "REGIONS5");
        //if the label is not the smallest label number of the region
        if (labelTable[i] != i) {
            if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "REGIONS6");
            ...
            if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "REGIONS8");
...
void setColors() {
    for (int i = 0; i < blobs.size();i++) {
        ...
        if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "X: %i -> %i, Y: %i -> %i, Color: %s (%i, %i, %i), Mass: %i", blobs[i].minX, blobs[i].maxX, blobs[i].minY, blobs[i].maxY, blobs[i].color.name.c_str(), blobs[i].color.r, blobs[i].color.g, blobs[i].color.b, blobs[i].mass);
...
void mergeDensities() {
    if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "MERGE0");
    for (int i = 0; i < (int) blobs.size() - 1;i++) {
        for (int j = i + 1; j < blobs.size(); j++) {
            if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "MERGE1");
            ...
            if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "MERGE2");
            ...
            if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "MERGE3");
        }
    }
    if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "MERGE4");
}
...
void detectColors(cv::Mat& image) {
    for (int y = 0; y < image.rows; y++) {
        for (int x = 0; x < image.cols; x++) {
            ...
            int color_u = image.at<cv::Vec3b>(y, x)[1];
            int color_v = image.at<cv::Vec3b>(y, x)[2];

            int colorBits = (y_arr[color_y] & u_arr[color_u]) & v_arr[color_v];
            
            if (x == 400 && y == 450) {
                if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, "TestColors", "Matching colors check new: (%i, %i, %i)", color_y, color_u, color_v);
                if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, "TestColors", "Array vals: (%i, %i, %i)", y_arr[color_y], u_arr[color_u], v_arr[color_v]);
            }
            
            //if (colorBits != 0) if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, "TestColors", "Matching colors check matched: (%i, %i, %i), %i x %i", color_y, color_u, color_v, x, y);
...
std::vector<char> createTestImage(cv::Mat& image) {
    ...
    if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "TESTIMAGE0");
    int i = 0;
    for (int y = 0; y < image.rows; y++) {
        for (int x = 0; x < image.cols; x++) {
    ...
    if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "TESTIMAGE4: %i", blobs.size());
    ...
    for (int i = 0; i < blobs.size(); i++) {
        ...
        if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "TESTIMAGE5");
        for (int x = minX; x <= maxX; x++) {
            //if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "TESTIMAGE6");
            testImage[4*(minY * image.cols + x)] = blobs[i].color.r;
            testImage[4*(minY * image.cols + x) + 1] = blobs[i].color.g;
            testImage[4*(minY * image.cols + x) + 2] = blobs[i].color.b;

            testImage[4*(maxY * image.cols + x) + 0] = blobs[i].color.r;
            testImage[4*(maxY * image.cols + x) + 1] = blobs[i].color.g;
            testImage[4*(maxY * image.cols + x) + 2] = blobs[i].color.b;
            //if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "TESTIMAGE7");
        }
        for (int y = minY; y <= maxY; y++) {
            //if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "TESTIMAGE8");
            testImage[4*(y * image.cols + minX)] = blobs[i].color.r;
            testImage[4*(y * image.cols + minX) + 1] = blobs[i].color.g;
            testImage[4*(y * image.cols + minX) + 2] = blobs[i].color.b;
            //if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "TESTIMAGE9");
            testImage[4*(y * image.cols + maxX) + 0] = blobs[i].color.r;
            testImage[4*(y * image.cols + maxX) + 1] = blobs[i].color.g;
            testImage[4*(y * image.cols + maxX) + 2] = blobs[i].color.b;
        }
    }
    if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "TESTIMAGE10");
    return testImage;
}

cv::Mat RGBtoYUV(cv::Mat& image) {
    cv::Mat converted = image.clone();
    cv::cvtColor(image, converted, CV_RGB2HSV);
    return converted;
}

//Get the blobs for the Java
std::vector<char> getBlobs(cv::Mat& image) {
    int color_y = image.at<cv::Vec4b>(450, 400)[0];
    int color_u = image.at<cv::Vec4b>(450, 400)[1];
    int color_v = image.at<cv::Vec4b>(450, 400)[2];
    if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, "TestColors", "Matching colors check rgb: (%i, %i, %i)", color_y, color_u, color_v);
    cv::Mat converted = RGBtoYUV(image);
    //cv::Mat converted = image.clone();
    loadColors();
    if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "HERE0");
    detectColors(converted);
    if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "HERE1");

    findRegions(converted);
    if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "HERE2");

    mergeDensities();
    if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "HERE3");

    setColors();
    if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "HERE4");
    
    std::vector<char> byteArray = createTestImage(image);
    if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "HERE5");
    return byteArray;
}
...
extern "C" {
    JNIEXPORT jobjectArray JNICALL Java_com_thereferees_opencvcamerarec_MainActivity_findObjects( JNIEnv* env, jobject, jlong addrImage) {
        ...
        if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "DONE!!!!! %i", blobs.size());
        for (int i = 0; i < blobs.size(); i++) {
            region* b = &blobs[i];
            ...
            if (TESTING) __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "RETURNED: [%i, %i, %i, %i, %i, %i]", b->minX, b->maxX, b->minY, b->maxY, b->mass, b->colorBits);
        }
